...
Func Example()
	Local $oDoc, $oDoc2, $oViewCursor, $oPar
	Local $aoPars[0]
	...
	If @error Then _ERROR("Failed to move the View Cursor. Error:" & @error & " Extended:" & @extended)
	...
	If @error Then _ERROR("Failed to set Font Color. Error:" & @error & " Extended:" & @extended)
	...
	If @error Then _ERROR("Failed to set underline settings. Error:" & @error & " Extended:" & @extended)

	; Create a list of Paragraph Objects
	$aoPars = _LOWriter_ParObjCreateList($oViewCursor)
	If @error Then _ERROR("Failed to retrieve array of paragraphs. Error:" & @error & " Extended:" & @extended)

	; Select the first Paragraph
	_LOWriter_ParObjSelect($oDoc, $aoPars[0])
	If @error Then _ERROR("Failed to select the paragraph. Error:" & @error & " Extended:" & @extended)

	; Copy the Paragraph.
	$oPar = _LOWriter_ParObjCopy($oDoc)
	If @error Then _ERROR("Failed to copy the paragraphs. Error:" & @error & " Extended:" & @extended)
	...
	If @error Then _ERROR("Failed to paste the paragraph object. Error:" & @error & " Extended:" & @extended)
