...
Func Example()
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "Press ok to delete the two middle rows.")

	; Delete some Table rows.
	_LOImpress_TableRowDelete($oTable, 1, 2)
	If @error Then _ERROR($oDoc, "Failed to delete Table Row. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
