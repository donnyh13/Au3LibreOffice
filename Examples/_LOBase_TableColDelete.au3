...
Func Example()
	...
	If @error Then Return _ERROR($oDoc, "Failed to retrieve array of Column names. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
	...
	If @error Then Return _ERROR($oDoc, "Failed to delete Column. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Table contains the following columns: " & @CRLF & $sNames)
