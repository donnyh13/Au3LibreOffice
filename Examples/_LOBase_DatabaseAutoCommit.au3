...
Func Example()
	...
	; Retrieve the current AutoCommit setting.
	$bAutoCommit = _LOBase_DatabaseAutoCommit($oConnection)
	If @error Then Return _ERROR($oDoc, "Failed to retrieve current AutoCommit setting. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "Are changes automatically committed for this connection? True/False: " & $bAutoCommit & @CRLF & _
