...
Func Example()
	...
	If @error Then _ERROR("Failed to Create a new Numbering Style. Error:" & @error & " Extended:" & @extended)

	; Check if the Numbering style exists.
	$bExists = _LOWriter_NumStyleExists($oDoc, "NewNumberingStyle")
	If @error Then _ERROR("Failed to test for Numbering Style existing in document. Error:" & @error & " Extended:" & @extended)
