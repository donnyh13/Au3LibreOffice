...
Func Example()
	Local $oDoc, $oForm, $oControl, $oParent
	Local $avProps[0]
	...
	; Retrieve the Form that contains the control (Parent).
	$oParent = _LOWriter_FormConGetParent($oControl)
	If @error Then _ERROR($oDoc, "Failed to retrieve form control's parent. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "The control's Parent Form has the name of: " & $avProps[0])
