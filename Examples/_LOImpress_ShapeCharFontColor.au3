...
Func Example()
	...
	; Set the Shape's font color to $LO_COLOR_RED, Transparency to 50%, and Highlight to $LO_COLOR_GOLD
	_LOImpress_ShapeCharFontColor($oTextBox, $LO_COLOR_RED, 50, $LO_COLOR_GOLD)
	...
	$avSettings = _LOImpress_ShapeCharFontColor($oTextBox)
	If @error Then _ERROR($oDoc, "Failed to retrieve the Shape's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Shape's current font color settings are as follows: " & @CRLF & _
			...
			"Current Font highlight color is (as a RGB Color Integer): " & $avSettings[2])
