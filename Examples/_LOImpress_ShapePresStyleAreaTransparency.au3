...
Func Example()
	...
	; Modify the Style Transparency settings to 55% transparent
	_LOImpress_ShapePresStyleAreaTransparency($oStyle, 55)
	If @error Then _ERROR($oDoc, "Failed to set Style settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve the current Style Transparency. Return will be an Integer.
	$iTransparency = _LOImpress_ShapePresStyleAreaTransparency($oStyle)
	If @error Then _ERROR($oDoc, "Failed to retrieve Style settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Style's current Transparency percentage is: " & $iTransparency)
