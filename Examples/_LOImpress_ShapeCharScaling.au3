...
Func Example()
	...
	; Set the Shape's Scale width to 75%
	_LOImpress_ShapeCharScaling($oTextBox, 75)
	...
	$iScale = _LOImpress_ShapeCharScaling($oTextBox)
	If @error Then _ERROR($oDoc, "Failed to retrieve the Shape's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Shape's current scale width is in percentage: " & $iScale)
