...
Func Example()
	...
	If @error Then _ERROR("Failed to retrieve next Redo action title. Error:" & @error & " Extended:" & @extended)

	MsgBox($MB_OK, "", "The next Redo action title is: " & $sRedo & " Press ok to perform it.")

	; Perform one Redo action.
	_LOWriter_DocRedo($oDoc)
	If @error Then _ERROR("Failed to perform an Redo action. Error:" & @error & " Extended:" & @extended)
