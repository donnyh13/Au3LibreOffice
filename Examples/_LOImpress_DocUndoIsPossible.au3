...
Func Example()
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "Is an Undo operation possible? True/False: " & _LOImpress_DocUndoIsPossible($oDoc))

	; Perform another undo action.
	_LOImpress_DocUndo($oDoc)
	If @error Then _ERROR($oDoc, "Failed to perform an undo action. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "Now is an Undo operation possible? True/False: " & _LOImpress_DocUndoIsPossible($oDoc))
