...
Func Example()
	...
	_LOWriter_DirFrmtUnderLine($oViewCursor, $LOW_UNDERLINE_WAVE)
	...
	_LOWriter_DirFrmtUnderLine($oViewCursor, $LOW_UNDERLINE_BOLD_DASH)
	If @error Then _ERROR($oDoc, "Failed to set the Selected text's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
	...
	; Create a Find Format Search Array for $LOW_UNDERLINE_BOLD_DASH underline style.
	_LOWriter_FindFormatModifyUnderline($atFindFormat, $LOW_UNDERLINE_BOLD_DASH)
	If @error Then _ERROR($oDoc, "Failed to modify a Find format array. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Search for the word "search" that is underlined using $LOW_UNDERLINE_BOLD_DASH line style.
	...
	If IsObj($oResult) Then
		...
		MsgBox($MB_OK + $MB_TOPMOST, Default, "The search was successful, I searched using a Find Format, looking for a underlined word, and found the following: " & $sResultString)
