...
Func Example()
	...
	; Set the Style's Font effects to $LOI_CHAR_RELIEF_EMBOSSED relief type.
	_LOImpress_ShapePresStyleCharEffect($oStyle, Null, $LOI_CHAR_RELIEF_EMBOSSED)
	...
	$avSettings = _LOImpress_ShapePresStyleCharEffect($oStyle)
	If @error Then _ERROR($oDoc, "Failed to retrieve the Style's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Style's current Font Effects settings are as follows: " & @CRLF & _
	...
	; Set the Style's Font effects to $LOI_CHAR_CASEMAP_SM_CAPS Case type type, with outlining.
	_LOImpress_ShapePresStyleCharEffect($oStyle, $LOI_CHAR_CASEMAP_SM_CAPS, $LOI_CHAR_RELIEF_NONE, True, True)
	If @error Then _ERROR($oDoc, "Failed to set the Style's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
