...
Func Example()
	...
	; Set the Cell's Scale width to 75%
	_LOImpress_TableCellCharScaling($oCell, 75)
	...
	$iScale = _LOImpress_TableCellCharScaling($oCell)
	If @error Then _ERROR($oDoc, "Failed to retrieve the Cell's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Cell's current scale width is in percentage: " & $iScale)
