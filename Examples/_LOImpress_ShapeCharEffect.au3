...
Func Example()
	...
	; Set the Text Box's Font effects to $LOI_CHAR_RELIEF_EMBOSSED relief type.
	_LOImpress_ShapeCharEffect($oTextBox, Null, $LOI_CHAR_RELIEF_EMBOSSED)
	...
	$avSettings = _LOImpress_ShapeCharEffect($oTextBox)
	If @error Then _ERROR($oDoc, "Failed to retrieve the Shape's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Shape's current Font Effects settings are as follows: " & @CRLF & _
	...
	; Set the Shape's Font effects to $LOI_CHAR_CASEMAP_SM_CAPS Case type type, with outlining.
	_LOImpress_ShapeCharEffect($oTextBox, $LOI_CHAR_CASEMAP_SM_CAPS, $LOI_CHAR_RELIEF_NONE, True, True)
	If @error Then _ERROR($oDoc, "Failed to set the Shape's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
