...
Func Example()
	...
	; Retrieve the Pivot Table's current name.
	$sName = _LOCalc_RangePivotName($oDoc, $oPivot)
	If @error Then _ERROR($oDoc, "Failed to retrieve Pivot Table's name. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Pivot Table's current name is: " & $sName & @CRLF & "Press ok to modify it.")

	; Modify the Pivot Table name to "Pivot_Testing"
	_LOCalc_RangePivotName($oDoc, $oPivot, "Pivot_Testing")
	If @error Then _ERROR($oDoc, "Failed to modify Pivot Table's name. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve the Pivot Table's name again.
	$sName = _LOCalc_RangePivotName($oDoc, $oPivot)
	If @error Then _ERROR($oDoc, "Failed to retrieve Pivot Table's name. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Pivot Table's new name is: " & $sName)
