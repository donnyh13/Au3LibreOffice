...
Func Example()
	...
	; Set the Cell's font position to auto Superscript, and relative size to 50%.
	_LOImpress_TableCellCharPosition($oCell, -1, Null, 50)
	...
	$avSettings = _LOImpress_TableCellCharPosition($oCell)
	If @error Then _ERROR($oDoc, "Failed to retrieve the Cell's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Cell's current position settings are as follows: " & @CRLF & _
