...
Func Example()
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "I am going to demonstrate how to find the Long color format integer value from R(ed), G(reen), B(lue) values, a Hexadecimal value, " & _
			" C(yan), M(agenta), Y(ellow), and K(ey) values, and H(ue), S(aturation) B(rightness) values, from the color constant $LOI_COLOR_MAGENTA. According to " & _
	...
	$iRGB_TO_LONG = _LOImpress_ConvertColorToLong(191, 0, 65)
	...
	$iHEX_TO_LONG = _LOImpress_ConvertColorToLong("bf0041")
	...
	$iCMYK_TO_LONG = _LOImpress_ConvertColorToLong(0, 100, 66, 25)
	...
	$iHSB_TO_LONG = _LOImpress_ConvertColorToLong("340", "100", "75")
