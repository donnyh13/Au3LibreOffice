...
Func Example()
	...
	If @error Then _ERROR($oDoc, $oDoc2, $oDoc3, "Failed to Create a new Calc Document. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Connect to the Current Document.
	$oDoc2 = _LOCalc_DocConnect($LO_DOC_CONNECT_MODE_CURRENT)
	If (@error > 0) Or Not IsObj($oDoc2) Then _ERROR($oDoc, $oDoc2, $oDoc3, "Failed to Connect to Calc Document. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve Doc Name.
	$sDocName = _LOCalc_DocGetName($oDoc2, False)
	If @error Then _ERROR($oDoc, $oDoc2, $oDoc3, "Failed to retrieve Calc Document name. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
...
Func _ERROR($oDoc, $oDoc2, $oDoc3, $sErrorText)
	...
	If IsObj($oDoc3) Then _LOCalc_DocClose($oDoc3, False)
	Exit
EndFunc
