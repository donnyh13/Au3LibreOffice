...
Func Example()
	...
	; Set the Cell's font color to $LO_COLOR_RED, Transparency to 50%, and Highlight to $LO_COLOR_GOLD
	_LOImpress_TableCellCharFontColor($oCell, $LO_COLOR_RED, 50, $LO_COLOR_GOLD)
	...
	$avSettings = _LOImpress_TableCellCharFontColor($oCell)
	If @error Then _ERROR($oDoc, "Failed to retrieve the Cell's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Cell's current font color settings are as follows: " & @CRLF & _
