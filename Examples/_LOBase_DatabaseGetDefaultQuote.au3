...
Func Example()
	...
	; Retrieve the default quotation character for the Database.
	$sQuote = _LOBase_DatabaseGetDefaultQuote($oConnection)
	If @error Then Return _ERROR($oDoc, "Failed to retrieve Default Quotation character for the Database. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The default quotation character for the Database is: " & $sQuote & @CRLF & _
