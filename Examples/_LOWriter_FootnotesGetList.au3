...
Func Example()
	...
	If (@error > 0) Then _ERROR("Failed to retrieve an array of Footnote objects. Error:" & @error & " Extended:" & @extended)

	;Delete the first Footnote returned
	_LOWriter_FootnoteDelete($aoFootnotes[0])
	If (@error > 0) Then _ERROR("Failed to delete a Footnote. Error:" & @error & " Extended:" & @extended)
