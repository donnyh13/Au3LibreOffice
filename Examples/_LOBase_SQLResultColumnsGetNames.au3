...
Func Example()
	...
	; Retrieve a count of Result Columns
	$asColumns = _LOBase_SQLResultColumnsGetNames($oResult)
	If @error Then Return _ERROR($oDoc, "Failed to retrieve an array of column names returned in the SQL Result Set. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	For $i = 0 To @extended - 1
		$sColumns &= $asColumns[$i] & @CRLF
	Next

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Result set contains the following Column names:" & @CRLF & $sColumns)
