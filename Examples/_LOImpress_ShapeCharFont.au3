...
Func Example()
	...
	; Set the Shape's Font to "Arial", Font size to 18, Posture (Italic) to $LOI_CHAR_POSTURE_ITALIC, and weight (Bold) to $LOI_CHAR_WEIGHT_BOLD
	_LOImpress_ShapeCharFont($oTextBox, "Arial", 18, $LOI_CHAR_POSTURE_ITALIC, $LOI_CHAR_WEIGHT_BOLD)
	...
	$avSettings = _LOImpress_ShapeCharFont($oTextBox)
	If @error Then _ERROR($oDoc, "Failed to retrieve the Shape's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Shape's current Font settings are as follows: " & @CRLF & _
