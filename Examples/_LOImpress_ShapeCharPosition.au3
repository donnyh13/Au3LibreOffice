...
Func Example()
	...
	; Set the Shape's font position to auto Superscript, and relative size to 50%.
	_LOImpress_ShapeCharPosition($oTextBox, -1, Null, 50)
	...
	$avSettings = _LOImpress_ShapeCharPosition($oTextBox)
	If @error Then _ERROR($oDoc, "Failed to retrieve the Shape's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Shape's current position settings are as follows: " & @CRLF & _
			...
			"Relative size percentage: " & $avSettings[2])
