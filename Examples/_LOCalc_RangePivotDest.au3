...
Func Example()
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "Press ok to change the pivot Table destination to begin at Cell E17.")

	; Retrieve the Cell where I will move thePivot Table output to.
	$oDestination = _LOCalc_RangeGetCellByName($oSheet, "E17")
	If @error Then _ERROR($oDoc, "Failed to retrieve Cell Object. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Modify the Pivot Table destination.
	_LOCalc_RangePivotDest($oDoc, $oPivot, $oDestination)
	If @error Then _ERROR($oDoc, "Failed to modify Pivot Table destination. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
