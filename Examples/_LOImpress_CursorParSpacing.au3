...
Func Example()
	...
	; Set the paragraph at the current cursor's location line space settings to, 1/4" above paragraph, 1/4" below paragraph,
	; Line space mode = $LOI_PAR_LINE_SPC_MODE_FIX, Line space height = 1/2".
	_LOImpress_CursorParSpacing($oTextCursor, $iHMM, $iHMM, $LOI_PAR_LINE_SPC_MODE_FIX, $iHMM2)
	If @error Then _ERROR($oDoc, "Failed to set the Selected text's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve the current settings. Return will be an array with element values in order of function parameters.
	$avSettings = _LOImpress_CursorParSpacing($oTextCursor)
