...
Func Example()
	...
	_LOWriter_DirFrmtCharOverLine($oViewCursor, Null, $LOW_CHAR_UNDERLINE_WAVE)
	...
	_LOWriter_DirFrmtCharOverLine($oViewCursor, Null, $LOW_CHAR_UNDERLINE_BOLD_DASH)
	If @error Then _ERROR($oDoc, "Failed to set the Selected text's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
	...
	If IsObj($oResult) Then
		...
		MsgBox($MB_OK + $MB_TOPMOST, Default, "The search was successful, I searched using a Find Format, looking for a overlined word, and found the following: " & $sResultString)
