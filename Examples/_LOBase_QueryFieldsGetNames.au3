...
Func Example()
	...
	_LOBase_TableColAdd($oTable, "Col2", $LOB_DATA_TYPE_INTEGER)
	...
	$oQuery = _LOBase_QueryAddBySQL($oConnection, "qryAutoIt_Query", "SELECT DISTINCT ""Col1"" ""Column_1"", ""Col2"" ""Column_2"" FROM ""tblNew_Table"" LIMIT 10")
	...
	; Retrieve an array of Query Field names.
	$asQueries = _LOBase_QueryFieldsGetNames($oQuery)
	If @error Then Return _ERROR($oDoc, "Failed to retrieve an array of Query field names. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	For $i = 0 To @extended - 1
		$sFields &= $asQueries[$i] & @CRLF
	Next

	MsgBox($MB_OK + $MB_TOPMOST, Default, "This Query contains the following Fields:" & @CRLF & $sFields)
