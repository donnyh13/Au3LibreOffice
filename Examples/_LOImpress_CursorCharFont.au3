...
Func Example()
	...
	; Set the selected text's Font to "Arial", Font size to 18, Posture (Italic) to $LOI_CHAR_POSTURE_ITALIC, and weight (Bold) to $LOI_CHAR_WEIGHT_BOLD
	_LOImpress_CursorCharFont($oTextCursor, "Arial", 18, $LOI_CHAR_POSTURE_ITALIC, $LOI_CHAR_WEIGHT_BOLD)
	If @error Then _ERROR($oDoc, "Failed to set the Selected text's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve the current settings. Return will be an array with element values in order of function parameters.
	$avSettings = _LOImpress_CursorCharFont($oTextCursor)
