...
Func Example()
	...
	; Check if a pivot table named "AutoIt_Pivot" exists.
	$bReturn = _LOCalc_RangePivotExists($oSheet, "AutoIt_Pivot")
	If @error Then _ERROR($oDoc, "Failed to query for Pivot Table name. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "Does a Pivot table with the name ""AutoIt_Pivot"" exist in this sheet? True/False: " & $bReturn)

	; Check if a pivot table named "Pivot123" exists.
	$bReturn = _LOCalc_RangePivotExists($oSheet, "Pivot123")
	If @error Then _ERROR($oDoc, "Failed to query for Pivot Table name. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "Does a Pivot table with the name ""Pivot123"" exist in this sheet? True/False: " & $bReturn)
