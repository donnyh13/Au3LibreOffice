...
Func Example()
	...
	; Set the Table's font color to $LO_COLOR_RED, Transparency to 50%, and Highlight to $LO_COLOR_GOLD
	_LOImpress_TableCharFontColor($oTable, $LO_COLOR_RED, 50, $LO_COLOR_GOLD)
	...
	$avSettings = _LOImpress_TableCharFontColor($oTable)
	If @error Then _ERROR($oDoc, "Failed to retrieve the Table's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Table's current font color settings are as follows: " & @CRLF & _
