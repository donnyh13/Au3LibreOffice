...
Func Example()
	...
	; Set the Shape's line space settings to, 1/4" above paragraph, 1/4" below paragraph,
	; Line space mode = $LOI_PAR_LINE_SPC_MODE_FIX, Line space height = 1/2".
	_LOImpress_ShapeParSpacing($oTextBox, $iHMM, $iHMM, $LOI_PAR_LINE_SPC_MODE_FIX, $iHMM2)
	...
	$avSettings = _LOImpress_ShapeParSpacing($oTextBox)
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "The current Paragraph Line Spacing settings are as follows: " & @CRLF & _
			...
			"Line spacing mode, (See UDF constants): " & $avSettings[2] & @CRLF & _
			"Line spacing distance, in Hundredths of a Millimeter (HMM) except for $LOI_PAR_LINE_SPC_MODE_PROP(0), which is a percentage: " & $avSettings[3])
