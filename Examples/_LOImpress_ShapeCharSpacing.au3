...
Func Example()
	...
	; Set the Shape's Kerning to 5.61
	_LOImpress_ShapeCharSpacing($oTextBox, False, 5.61)
	...
	$avSettings = _LOImpress_ShapeCharSpacing($oTextBox)
	If @error Then _ERROR($oDoc, "Failed to retrieve the Shape's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Shape's current kerning settings are as follows: " & @CRLF & _
			"Automatically adjust kerning? True/False: " & $avSettings[0] & @CRLF & _
			"Kerning value in Printer's Points: " & $avSettings[1])
