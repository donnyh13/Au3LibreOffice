...
Func Example()
	...
	_LOBase_ReportConInsert($oSection, $LOB_REP_CON_TYPE_TEXT_BOX, 10500, 500, 4500, 1500)
	...
	; Insert a control in the Page Header Section.
	_LOBase_ReportConInsert($oSection, $LOB_REP_CON_TYPE_LABEL, 1500, 300, 4500, 1500)
	If @error Then Return _ERROR($oDoc, $oReportDoc, "Failed to insert a Control. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve list of all controls contained in the Detail Section.
	$avCon = _LOBase_ReportConsGetList($oSection, $LOB_REP_CON_TYPE_ALL)
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "Press Ok to delete all controls contained in the Detail section of the Report.")

	For $i = 0 To UBound($avCon) - 1
		; Delete the Control.
		_LOBase_ReportConDelete($avCon[$i][0])
		If @error Then Return _ERROR($oDoc, $oReportDoc, "Failed to delete the Control. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
	Next
