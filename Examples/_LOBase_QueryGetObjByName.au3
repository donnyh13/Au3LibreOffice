...
Func Example()
	...
	_LOBase_QueryAddBySQL($oConnection, "qryAutoIt_Query", "SELECT DISTINCT ""Col1"" ""Column_1"" FROM ""tblNew_Table"" LIMIT 10")
	...
	; Retrieve the Query named "qryAutoIt_Query".
	$oQuery = _LOBase_QueryGetObjByName($oConnection, "qryAutoIt_Query")
	If @error Then Return _ERROR($oDoc, "Failed to retrieve Query Object. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve the Query's SQL command
	$sSQL = _LOBase_QuerySQLCommand($oQuery)
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "This Query's SQL command is: " & $sSQL)
