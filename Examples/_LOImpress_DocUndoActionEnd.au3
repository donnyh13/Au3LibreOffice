...
Func Example()
	...
	; Begin an Undo Group
	_LOImpress_DocUndoActionBegin($oDoc)
	If @error Then _ERROR($oDoc, "Failed to Begin undo group. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
	...
	; End the Undo Group
	_LOImpress_DocUndoActionEnd($oDoc)
	If @error Then _ERROR($oDoc, "Failed to end undo group. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "Press ok to perform an undo action once, which will undo both the Date Field and the new page insertion.")
