...
Func Example()
	...
	; Add a Query to the Document.
	_LOBase_QueryAddBySQL($oConnection, "qryNew_AutoIt_Query", "SELECT ""Col1"" FROM ""tblNew_Table""")
	If @error Then Return _ERROR($oDoc, "Failed to add a Query to the Database. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve an array of Query names.
	$asQueries = _LOBase_QueriesGetNames($oConnection)
	If @error Then Return _ERROR($oDoc, "Failed to retrieve an array of Query names in the Database. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	For $i = 0 To @extended - 1
		$sQueries &= $asQueries[$i] & @CRLF
	Next

	MsgBox($MB_OK + $MB_TOPMOST, Default, "Here is a list of all queries contained in the document:" & @CRLF & $sQueries & @CRLF & @CRLF & "I will now delete the ""qryAutoIt_Query"" Query.")

	; Delete the Query
	_LOBase_QueryDelete($oConnection, $oQuery)
	If @error Then Return _ERROR($oDoc, "Failed to delete Query. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "Here is a list of all queries contained in the document:" & @CRLF & $sQueries)
