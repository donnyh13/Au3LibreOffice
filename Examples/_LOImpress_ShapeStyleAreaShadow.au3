...
Func Example()
	...
	_LOImpress_ShapeStyleAreaColor($oStyle, $LO_COLOR_TEAL)
	...
	_LOImpress_ShapeStyleAreaShadow($oStyle, True, $LOI_SHAPE_SHADOW_LOCATION_BOTTOM_CENTER, $LO_COLOR_GOLD, 600, 25, 35)
	If @error Then _ERROR($oDoc, "Failed to set Style settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve the Style's current settings. Return will be an array in order of function parameters.
	$avSettings = _LOImpress_ShapeStyleAreaShadow($oStyle)
