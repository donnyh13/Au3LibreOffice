...
Func Example()
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "Press ok to insert the Pivot Table.")

	; Insert the Pivot Table. I will not pre-set up the Fields etc to show the result.
	$oPivot = _LOCalc_RangePivotInsert($oCellRange, $oDestination, "AutoIt_Pivot")
	If @error Then _ERROR($oDoc, "Failed to insert Pivot Table. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve the Object for Field "Item".
	$oField = _LOCalc_RangePivotFieldGetObjByName($oPivot, "Item")
	...
	; Set the Field/Column "Item" as a Column Field.
