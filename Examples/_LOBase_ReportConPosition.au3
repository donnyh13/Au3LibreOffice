...
Func Example()
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "Press Ok to move the new control.")

	; Move the Control.
	_LOBase_ReportConPosition($oControl, 1500, 850)
	If @error Then Return _ERROR($oDoc, $oReportDoc, "Failed to move a Control. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve the current position for the control. Return will be an Array in order of function parameters.
	$avControl = _LOBase_ReportConPosition($oControl)
	If @error Then _ERROR($oDoc, $oReportDoc, "Failed to retrieve Control's position values. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Control's current position values are: " & @CRLF & _
			"X: " & $avControl[0] & @CRLF & _
			"Y: " & $avControl[1])
