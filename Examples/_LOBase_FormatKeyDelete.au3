...
Func Example()
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "I created a new Number format key. Its Format Key number is: " & $iFormatKey & " It looks like this: " & _
			_LOBase_FormatKeyGetString($oReportDoc, $iFormatKey) & @CRLF & @CRLF & "Press Ok to delete it.")

	; Delete the Format Key.
	_LOBase_FormatKeyDelete($oReportDoc, $iFormatKey)
	If @error Then _ERROR($oDoc, $oReportDoc, "Failed to delete a Format Key. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "Does the document still have the Number format I created? True/False: " & _LOBase_FormatKeyExists($oReportDoc, $iFormatKey))
