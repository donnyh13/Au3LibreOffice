...
Func Example()
	...
	; Set some of the Note Page's Header settings
	_LOImpress_SlideNotesHeader($oNotes, True, "Hello, from AutoIt!", True, False, Null, $LOI_FIELD_DATE_FMT_DOW_MMM_DD_YYYY)
	If @error Then _ERROR($oDoc, "Failed to set Page's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve the Note Page's Header settings. Return will be an array in order of function parameters.
	$avSettings = _LOImpress_SlideNotesHeader($oNotes)
	If @error Then _ERROR($oDoc, "Failed to retrieve Page's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Note Page's Header settings are as follows: " & @CRLF & _
