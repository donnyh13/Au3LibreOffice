...
Func Example()
	...
	If (@error > 0) Then _ERROR("Failed to turn footers on for this Page Style. Error:" & @error & " Extended:" & @extended)

	;Create a text cursor for the page style footer.
	$oFooterTextCursor = _LOWriter_DocFooterGetTextCursor($oPageStyle, True)
	If (@error > 0) Then _ERROR("Failed to create a TextCursor in the Page Style's Footer. Error:" & @error & " Extended:" & @extended)
