...
Func Example()
	...
	$iColumns = _LOBase_SQLResultColumnsGetCount($oResult)
	If @error Then Return _ERROR($oDoc, "Failed to retrieve a count of columns returned in the SQL Result Set. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Result set contains " & $iColumns & " Columns.")
