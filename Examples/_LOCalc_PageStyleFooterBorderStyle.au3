...
Func Example()
	...
	; Set Page style Footer Border Style settings to: Top = $LOC_BORDER_STYLE_DASH_DOT_DOT, Bottom = $LOC_BORDER_STYLE_THICKTHIN_MEDIUMGAP
	; Left = $LOC_BORDER_STYLE_DOUBLE, Right = $LOC_BORDER_STYLE_DASHED
	_LOCalc_PageStyleFooterBorderStyle($oPageStyle, $LOC_BORDER_STYLE_DASH_DOT_DOT, $LOC_BORDER_STYLE_THICKTHIN_MEDIUMGAP, $LOC_BORDER_STYLE_DOUBLE, $LOC_BORDER_STYLE_DASHED)
	If @error Then _ERROR($oDoc, "Failed to modify Page Style settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve the current settings. Return will be an array with elements in order of function parameters.
	$avPageStyleSettings = _LOCalc_PageStyleFooterBorderStyle($oPageStyle)
