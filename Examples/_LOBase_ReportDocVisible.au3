...
Func Example()
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "I have opened the Report called ""rptAutoIt_Report"" in Design Mode. Press ok to make it invisible.")

	; Make the Report Document invisible.
	_LOBase_ReportDocVisible($oReportDoc, False)
	If @error Then Return _ERROR($oDoc, "Failed to make the Report Document invisible. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; See if the Report is visible.
	$bVisible = _LOBase_ReportDocVisible($oReportDoc)
	If @error Then Return _ERROR($oDoc, "Failed to query Report Document visibility. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "Is the Report currently visible? True/False: " & $bVisible & @CRLF & "Press ok to make it visible.")

	; Make the Report Document visible.
	_LOBase_ReportDocVisible($oReportDoc, True)
	If @error Then Return _ERROR($oDoc, "Failed to make the Report Document visible. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
