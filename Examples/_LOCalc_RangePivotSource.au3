...
Func Example()
	...
	; Retrieve the current source range for the Pivot Table
	$oCellRange = _LOCalc_RangePivotSource($oDoc, $oPivot)
	If @error Then _ERROR($oDoc, "Failed to retrieve Pivot Table source range. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Pivot Table's current source range is: " & _LOCalc_RangeGetAddressAsName($oCellRange) & @CRLF & "Press ok to modify it.")

	; Retrieve the range containing the Data, but leave out the last two years.
	$oCellRange = _LOCalc_RangeGetCellByName($oSheet, "A1", "F11")
	If @error Then _ERROR($oDoc, "Failed to retrieve Cell Range Object. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Set the Pivot Table to use the new range.
	_LOCalc_RangePivotSource($oDoc, $oPivot, $oCellRange)
	If @error Then _ERROR($oDoc, "Failed to set Cell Range Object. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve the current source range for the Pivot Table
	$oCellRange = _LOCalc_RangePivotSource($oDoc, $oPivot)
	If @error Then _ERROR($oDoc, "Failed to retrieve Pivot Table source range. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Pivot Table's new source range is: " & _LOCalc_RangeGetAddressAsName($oCellRange))
