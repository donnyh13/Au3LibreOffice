...
Func Example()
	...
	; Set the Shape's Strikeout settings to, Line style = $LOI_CHAR_STRIKEOUT_X, Words only = True.
	_LOImpress_ShapeCharStrikeOut($oTextBox, $LOI_CHAR_STRIKEOUT_X, True)
	...
	$avSettings = _LOImpress_ShapeCharStrikeOut($oTextBox)
	If @error Then _ERROR($oDoc, "Failed to retrieve the Shape's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Shape's current Strikeout settings are as follows: " & @CRLF & _
