...
Func Example()
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "Press ok to delete the pivot Table.")

	; Delete the Pivot Table.
	_LOCalc_RangePivotDelete($oDoc, $oPivot)
	If @error Then _ERROR($oDoc, "Failed to delete Pivot Table. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
