...
Func Example()
	...
	_LOWriter_FormConFileSelFieldValue($oControl, @ScriptDir & "\FakeFile.au3")
	If @error Then _ERROR($oDoc, "Failed to modify the Control's Value. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve the current Value of the control. Return will be a String.
	$sValue = _LOWriter_FormConFileSelFieldValue($oControl)
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Control's current value is: " & @CRLF & $sValue)
