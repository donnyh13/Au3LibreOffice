...
Func Example()
	...
	Local $sSavePath, $sSubComponents = ""
	Local $avSubComponents[0][4]
	...
	_LOBase_TableDocOpenByObject($oDoc, $oConnection, $oTable)
	If @error Then Return _ERROR($oDoc, "Failed to open Table Document. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Open the Table Document in Design Mode.
	_LOBase_TableDocOpenByObject($oDoc, $oConnection, $oTable, True)
	If @error Then Return _ERROR($oDoc, "Failed to open Table Document. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
	...
	_LOBase_QueryDocOpenByObject($oConnection, $oQuery)
	If @error Then Return _ERROR($oDoc, "Failed to open Query Document. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Open the Query Document in Design mode.
	_LOBase_QueryDocOpenByObject($oConnection, $oQuery, True)
	If @error Then Return _ERROR($oDoc, "Failed to open Query Document. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
	...
	; Retrieve an array of open SubComponents.
	$avSubComponents = _LOBase_DocSubComponentsGetList($oDoc)
	If @error Then Return _ERROR($oDoc, "Failed to retrieve array of SubComponents. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	For $i = 0 To @extended - 1
		$sSubComponents &= "Column 0 -- " & ((IsObj($avSubComponents[$i][0])) ? ("[Object]") : ("[Not An Object]")) & @CRLF
		$sSubComponents &= "Column 1 -- " & "SubComponent Type: " & $avSubComponents[$i][1] & @CRLF
		$sSubComponents &= "Column 2 -- " & "SubComponent Name: " & $avSubComponents[$i][2] & @CRLF
		$sSubComponents &= "Column 3 -- " & "SubComponent Is Open in Design mode? True/False: " & $avSubComponents[$i][3] & @CRLF & @CRLF
	Next

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The following SubComponents are opened:" & @CRLF & $sSubComponents)
