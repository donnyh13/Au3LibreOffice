...
Func Example()
	...
	$avSettings = _LOCalc_RangePivotFieldSettings($oField)
	If @error Then _ERROR($oDoc, "Failed to retrieve the Pivot Table Field's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The ""2012"" Pivot Table Field's settings are as follows: " & @CRLF & _
			"The type of field is (See UDF Constants): " & $avSettings[0] & @CRLF & _
			"The function applied to the field is (See UDF Constants): " & $avSettings[1] & @CRLF & _
			"Are empty values included in the results? True/False: " & $avSettings[2] & @CRLF & _
			"What type of calculations are done to the results? (See UDF Constants): " & $avSettings[3] & @CRLF & _
			"The Field's name the the calculations are based on is: " & $avSettings[4] & @CRLF & _
			"The type of base item to base the calculation on is (See UDF Constants): " & $avSettings[5] & @CRLF & _
			"The base item name to base the calculation on is: " & $avSettings[6])
