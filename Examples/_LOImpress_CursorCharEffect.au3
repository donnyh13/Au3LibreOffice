...
Func Example()
	...
	; Set the selected text's Font effects to $LOI_CHAR_RELIEF_EMBOSSED relief type.
	_LOImpress_CursorCharEffect($oTextCursor, Null, $LOI_CHAR_RELIEF_EMBOSSED)
	If @error Then _ERROR($oDoc, "Failed to set the Selected text's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve the current settings. Return will be an array with element values in order of function parameters.
	$avSettings = _LOImpress_CursorCharEffect($oTextCursor)
	...
	; Set the selected text's Font effects to $LOI_CHAR_CASEMAP_SM_CAPS Case type type, with outlining.
	_LOImpress_CursorCharEffect($oTextCursor, $LOI_CHAR_CASEMAP_SM_CAPS, $LOI_CHAR_RELIEF_NONE, True, True)
	If @error Then _ERROR($oDoc, "Failed to set the Selected text's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
