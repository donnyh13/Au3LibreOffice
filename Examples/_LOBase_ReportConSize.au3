...
Func Example()
	...
	$oControl = _LOBase_ReportConInsert($oSection, $LOB_REP_CON_TYPE_TEXT_BOX, 10500, 500, 4500, 1500)
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "Press Ok to change the control's size.")

	; Change the Control's size.
	_LOBase_ReportConSize($oControl, 3000, 2500, False)
	If @error Then Return _ERROR($oDoc, $oReportDoc, "Failed to modify a Control's size. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve the current size of the control. Return will be an Array in order of function parameters.
	$avControl = _LOBase_ReportConSize($oControl)
	If @error Then _ERROR($oDoc, $oReportDoc, "Failed to retrieve Control's size values. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Control's current size values are: " & @CRLF & _
			"Width: " & $avControl[0] & @CRLF & _
			"Height: " & $avControl[1] & @CRLF & _
			"Will the Control AutoGrow to fit content? True/False: " & $avControl[2])
