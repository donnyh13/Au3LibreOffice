...
Func Example()
	...
	; Set the selected text's underline settings to Underline style $LOI_CHAR_UNDERLINE_BOLD_DASH_DOT, Color to $LO_COLOR_BROWN, and Words only = True
	_LOImpress_CursorCharUnderLine($oTextCursor, $LOI_CHAR_UNDERLINE_BOLD_DASH_DOT, $LO_COLOR_BROWN, True)
	If @error Then _ERROR($oDoc, "Failed to set the Selected text's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve the current settings. Return will be an array with element values in order of function parameters.
	$avSettings = _LOImpress_CursorCharUnderLine($oTextCursor)
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "The selected text's current underline settings are as follows: " & @CRLF & _
