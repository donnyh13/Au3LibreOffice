...
Func Example()
	...
	_LOWriter_FormConTableConListBoxData($oColumn, "Testing", True, $LOW_FORM_CON_SOURCE_TYPE_VALUE_LIST, $asValues)
	If @error Then _ERROR($oDoc, "Failed to set column's Data values. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve the current settings for the column. Return will be an Array in order of function parameters.
	$avColumn = _LOWriter_FormConTableConListBoxData($oColumn)
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Column's current settings are: " & @CRLF & _
			...
			"Is user input required? True/False: " & $avColumn[1] & @CRLF & _
			"What type of content is the List box filled with? (See UDF Constants): " & $avColumn[2] & @CRLF & _
			"This is an array of entries. I'll just see the size of the array: " & UBound($avColumn[3]) & @CRLF & _
			"The bound field value is: " & $avColumn[4] & @CRLF & @CRLF & _
			"Press ok to set values for a Table now.")

	; Modify the Column's Data properties.
	_LOWriter_FormConTableConListBoxData($oColumn, "Datafield1", False, $LOW_FORM_CON_SOURCE_TYPE_TABLE, $asTables, 4)
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Column's current settings are: " & @CRLF & _
			...
			"What type of content is the List box filled with? (See UDF Constants): " & $avColumn[2] & @CRLF & _
			"This is an array of entries. I'll just see the size of the array: " & UBound($avColumn[3]) & @CRLF & _
			"The bound field value is: " & $avColumn[4])
