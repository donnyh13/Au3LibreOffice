...
Func Example()
	Local $oDoc, $oSlide, $oTextBox, $oTextCursor, $oTextCursor2
	Local $avShapes
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "Press ok to create another TextCursor and insert text where the first cursor is.")

	; Create another Text Cursor in the Textbox.
	$oTextCursor2 = _LOImpress_ShapeCreateTextCursor($oTextBox)
	If @error Then _ERROR($oDoc, "Failed to create a Text Cursor. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Move the second TextCursor to where the first TextCursor is, do not select text on the way.
	_LOImpress_CursorGoToRange($oTextCursor2, $oTextCursor, False)
	If @error Then _ERROR($oDoc, "Failed to move Text Cursor. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Insert some text.
	_LOImpress_CursorInsertString($oTextCursor2, " This is where the second TextCursor is!")
	If @error Then _ERROR($oDoc, "Failed to insert some text. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
