...
Func Example()
	...
	_LOImpress_TableCellBorderStyle($oCell, $LOI_SHAPE_BORDER_STYLE_DOTTED, $LOI_SHAPE_BORDER_STYLE_DASHED, $LOI_SHAPE_BORDER_STYLE_DASH_DOT_DOT, $LOI_SHAPE_BORDER_STYLE_THICKTHIN_SMALLGAP)
	If @error Then _ERROR($oDoc, "Failed to set Cell Border Style settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve current Border Style settings. Return will be an array in order of function parameters.
	$avSettings = _LOImpress_TableCellBorderStyle($oCell)
	If @error Then _ERROR($oDoc, "Failed to retrieve Cell Border Style settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Cell's Border Style settings are: " & @CRLF & _
