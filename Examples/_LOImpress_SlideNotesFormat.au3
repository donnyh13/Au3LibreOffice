...
Func Example()
	...
	; Set some of the Note Page's Format settings
	_LOImpress_SlideNotesFormat($oNotes, $LOI_PAGE_WIDTH_DIA_SLIDE, $LOI_PAGE_HEIGHT_DIA_SLIDE, $LOI_PAGE_ORIENT_PORTRAIT)
	If @error Then _ERROR($oDoc, "Failed to set Page's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve the Note Page's Format settings. Return will be an array in order of function parameters.
	$avSettings = _LOImpress_SlideNotesFormat($oNotes)
	If @error Then _ERROR($oDoc, "Failed to retrieve Page's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Note Page's format settings are as follows: " & @CRLF & _
