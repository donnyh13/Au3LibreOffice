...
Func Example()
	...
	; Retrieve the Field's current display value.
	$sFieldDisplay = _LOImpress_FieldCurrentDisplayGet($oField)
	If @error Then _ERROR($oDoc, "Failed to retrieve field's current display value. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Field's current value is: " & $sFieldDisplay)
