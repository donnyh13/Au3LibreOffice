...
Func Example()
	...
	; Retrieve middle Table Cell Object
	$oCell = _LOImpress_TableCellGetObjByPosition($oTable, 1, 1)
	If @error Then _ERROR($oDoc, "Failed to retrieve Table cell Object. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve the Cell String.
	$sString = _LOImpress_TableCellString($oCell)
	If @error Then _ERROR($oDoc, "Failed to retrieve Table cell String. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The middle Cell's String is: " & $sString)
