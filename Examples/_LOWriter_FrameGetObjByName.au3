...
Func Example()
	...
	If @error Then _ERROR("Failed to retrieve a list of Frame. Error:" & @error & " Extended:" & @extended)
	...
	If (UBound($asFrames) > 0) Then
		...
		If @error Then _ERROR("Failed to delete a frame. Error:" & @error & " Extended:" & @extended)

	Else
		_ERROR("Something went wrong, and no frames were found.")
	EndIf
