...
Func Example()
	...
	; Retrieve the current Table Fill Style
	$iFillStyle = _LOImpress_TableBackFillStyle($oTable)
	If @error Then _ERROR($oDoc, "Failed to retrieve Table Fill Style. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Table's current Fill Style is: " & $iFillStyle & @CRLF & _
