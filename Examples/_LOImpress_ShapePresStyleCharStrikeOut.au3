...
Func Example()
	...
	; Set the Style's Strikeout settings to, Line style = $LOI_CHAR_STRIKEOUT_X, Words only = True
	_LOImpress_ShapePresStyleCharStrikeOut($oStyle, $LOI_CHAR_STRIKEOUT_X, True)
	...
	$avSettings = _LOImpress_ShapePresStyleCharStrikeOut($oStyle)
	If @error Then _ERROR($oDoc, "Failed to retrieve the Style's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Style's current Strikeout settings are as follows: " & @CRLF & _
