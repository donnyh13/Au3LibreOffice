...
Func Example()
	...
	; Set the Table's Strikeout settings to, Line style = $LOI_CHAR_STRIKEOUT_X, Words only = True.
	_LOImpress_TableCharStrikeOut($oTable, $LOI_CHAR_STRIKEOUT_X, True)
	...
	$avSettings = _LOImpress_TableCharStrikeOut($oTable)
	If @error Then _ERROR($oDoc, "Failed to retrieve the Table's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Table's current Strikeout settings are as follows: " & @CRLF & _
