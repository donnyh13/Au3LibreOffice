...
Func Example()
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "I will now select all the words, and set underline style, Dash Dot, underline in the color Gold, and underline only words to True.")
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "The current Underline settings at the Cursor's current position are as follows: " & @CRLF & _
			"The Underline style is (See UDF Constants): " & $avSettings[0] & @CRLF & _
			"The underline color is (as a RGB Color Integer): " & $avSettings[1] & @CRLF & _
			"Are words only underlined? True/False: " & $avSettings[2])
