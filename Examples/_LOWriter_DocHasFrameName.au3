...
Func Example()
	...
	If @error Then _ERROR("Failed to look for Text Frame name. Error:" & @error & " Extended:" & @extended)

	MsgBox($MB_OK, "", "Does this document contain a Frame named ""AutoitTest""? True/ False. " & $bReturn)

	; Delete the Frame.
	_LOWriter_FrameDelete($oDoc, $oFrame)
	If @error Then _ERROR("Failed to delete Text Frame. Error:" & @error & " Extended:" & @extended)
