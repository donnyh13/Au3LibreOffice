...
Func Example()
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "Press ok to insert two columns at the end of the table.")

	; Insert some Table columns.
	_LOImpress_TableColumnInsert($oTable, 2, $iColumns)
	If @error Then _ERROR($oDoc, "Failed to insert a Table Column. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
