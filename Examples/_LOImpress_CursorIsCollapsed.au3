...
Func Example()
	...
	; Check if the Cursor's selection is currently collapsed, (data is selected).
	$bReturn = _LOImpress_CursorIsCollapsed($oTextCursor)
	...
	; Deselect all data.
	_LOImpress_CursorMove($oTextCursor, $LOI_TEXTCUR_GO_RIGHT, 0, False)
	If @error Then _ERROR($oDoc, "Failed to move Cursor. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
