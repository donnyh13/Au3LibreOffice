...
Func Example()
	...
	$avSettings = _LOImpress_TableCellBackGradient($oDoc, $oCell)
	...
	; Modify the Cell's Gradient settings to: skip pre-set gradient name, Gradient type = $LOI_GRAD_TYPE_SQUARE, increment steps = 150,
	...
	_LOImpress_TableCellBackGradient($oDoc, $oCell, Null, $LOI_GRAD_TYPE_SQUARE, 150, 25, 56, 135, 50, $LO_COLOR_ORANGE, $LO_COLOR_TEAL, 100, 68)
	If @error Then _ERROR($oDoc, "Failed to set Cell settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
