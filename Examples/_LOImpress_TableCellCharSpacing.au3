...
Func Example()
	...
	; Set the Cell's Kerning to 5.61
	_LOImpress_TableCellCharSpacing($oCell, False, 5.61)
	...
	$avSettings = _LOImpress_TableCellCharSpacing($oCell)
	If @error Then _ERROR($oDoc, "Failed to retrieve the Cell's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Cell's current kerning settings are as follows: " & @CRLF & _
