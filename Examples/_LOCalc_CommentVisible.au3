...
Func Example()
	...
	; Retrieve the current visibility of the Comment.
	$bVisible = _LOCalc_CommentVisible($oComment)
	If @error Then _ERROR($oDoc, "Failed to retrieve Comment settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "Is the Comment currently always visible? True/False: " & $bVisible & @CRLF & _
			"Press Ok to make the comment always visible.")
