...
Func Example()
	...
	; Set the Table's Overline settings to Overline style $LOI_CHAR_UNDERLINE_BOLD_DASH_DOT, Color to $LO_COLOR_BROWN, and Words only = True
	_LOImpress_TableCharOverLine($oTable, $LOI_CHAR_UNDERLINE_BOLD_DASH_DOT, $LO_COLOR_BROWN, True)
	...
	$avSettings = _LOImpress_TableCharOverLine($oTable)
	If @error Then _ERROR($oDoc, "Failed to retrieve the Table's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Table's current Overline settings are as follows: " & @CRLF & _
