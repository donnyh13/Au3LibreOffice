...
Func Example()
	...
	; Set the Style's Alignment settings to, Horizontal alignment = $LOI_PAR_ALIGN_HOR_JUSTIFIED,
	; Last line alignment = $LOI_PAR_LAST_LINE_JUSTIFIED, Text direction = $LOI_PAR_TXT_DIR_LR_TB
	_LOImpress_ShapePresStyleParAlignment($oStyle, $LOI_PAR_ALIGN_HOR_JUSTIFIED, $LOI_PAR_LAST_LINE_JUSTIFIED, $LOI_PAR_TXT_DIR_LR_TB)
	...
	$avSettings = _LOImpress_ShapePresStyleParAlignment($oStyle)
	If @error Then _ERROR($oDoc, "Failed to retrieve the Style's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Style's Alignment settings are as follows: " & @CRLF & _
