...
Func Example()
	...
	; Modify the settings for the Group Footer.
	_LOBase_ReportGroupFooter($oGroup, True, "AutoIt_Group", $LOB_REP_FORCE_PAGE_AFTER_SECTION, False, True, True, 2576, Null, $LO_COLOR_BRICK)
	...
	$avGroup = _LOBase_ReportGroupFooter($oGroup)
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Group's current settings are: " & @CRLF & _
			"Is the Group Footer enabled? True/False: " & $avGroup[0] & @CRLF & _
			"The Group's name is: " & $avGroup[1] & @CRLF & _
			"Will a new page be created before/after this Group Footer? If so, when? (See UDF Constants): " & $avGroup[2] & @CRLF & _
			"Will the content be prevented from spreading across more than one page? True/False: " & $avGroup[3] & @CRLF & _
			"Will the Footer be repeated on each page this section spans? True/False: " & $avGroup[4] & @CRLF & _
			"Is the Group Footer visible? True/False: " & $avGroup[5] & @CRLF & _
			"The Height of the Group Footer is, in Hundredths of a Millimeter (HMM): " & $avGroup[6] & @CRLF & _
