...
Func Example()
	...
	; Add the new Index, set Unique to False.
	_LOBase_TableIndexAdd($oTable, "Item_Index", $avColumns, False)
	...
	$asIndexes = _LOBase_TableIndexesGetNames($oTable)
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Indexes contained in this table are:" & @CRLF & $sIndexes & @CRLF & @CRLF & "Press Ok to delete the ""Item_Index"".")

	; Delete the Index "Item_Index".
	_LOBase_TableIndexDelete($oTable, "Item_Index")
	If @error Then Return _ERROR($oDoc, "Failed to delete index. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Indexes contained in this table now are:" & @CRLF & $sIndexes)
