...
Func Example()
	...
	If (@error > 0) Then _ERROR("Failed to set the row background color settings. Error:" & @error & " Extended:" & @extended)

	;Retrieve row's current background color settings.
	$avColor = _LOWriter_TableRowColor($oTable, 2)
	If (@error > 0) Then _ERROR("Failed to retrieve the row background color settings. Error:" & @error & " Extended:" & @extended)
