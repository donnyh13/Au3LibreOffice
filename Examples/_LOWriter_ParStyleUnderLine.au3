...
Func Example()
	...
	; Set Default Paragraph Style underline settings to Underline style $LOW_UNDERLINE_BOLD_DASH_DOT, Color to $LO_COLOR_BROWN, and Words only = True
	_LOWriter_ParStyleUnderLine($oParStyle, $LOW_UNDERLINE_BOLD_DASH_DOT, $LO_COLOR_BROWN, True)
	...
	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Paragraph's current underline settings are as follows: " & @CRLF & _
			"Underline style (See UDF constants): " & $avParStyleSettings[0] & @CRLF & _
			"Underline color is (as a RGB Color Integer): " & $avParStyleSettings[1] & @CRLF & _
			"Underline words only? True/False: " & $avParStyleSettings[2])
