...
Func Example()
	...
	; Set some of the Note Page's margin settings
	_LOImpress_SlideNotesMargins($oNotes, 1270, 2540, 2540, 1270)
	If @error Then _ERROR($oDoc, "Failed to set Page's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve the Note Page's margin settings. Return will be an array in order of function parameters.
	$avSettings = _LOImpress_SlideNotesMargins($oNotes)
	If @error Then _ERROR($oDoc, "Failed to retrieve Page's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Note Page's margin settings are as follows: " & @CRLF & _
