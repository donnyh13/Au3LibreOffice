...
Func Example()
	...
	; Retrieve the Style's current settings. Return will be an Integer.
	$iColor = _LOImpress_ShapeStyleAreaColor($oStyle)
	If @error Then _ERROR($oDoc, "Failed to retrieve Style's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Drawing Style's Background color settings are as follows: " & @CRLF & _
			"The Style's Background color is (as a RGB Color Integer): " & $iColor)
