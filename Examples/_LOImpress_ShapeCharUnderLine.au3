...
Func Example()
	...
	; Set the Shape's underline settings to Underline style $LOI_CHAR_UNDERLINE_BOLD_DASH_DOT, Color to $LO_COLOR_BROWN, and Words only = True
	_LOImpress_ShapeCharUnderLine($oTextBox, $LOI_CHAR_UNDERLINE_BOLD_DASH_DOT, $LO_COLOR_BROWN, True)
	...
	$avSettings = _LOImpress_ShapeCharUnderLine($oTextBox)
	If @error Then _ERROR($oDoc, "Failed to retrieve the Shape's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Shape's current underline settings are as follows: " & @CRLF & _
