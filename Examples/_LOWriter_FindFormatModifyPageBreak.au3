...
Func Example()
	...
	; Set the paragraph to Page Break Before this paragraph.
	_LOWriter_DirFrmtParPageBreak($oDoc, $oViewCursor, $LOW_BREAK_PAGE_BEFORE)
	...
	; Create a Find Format Search Array for Page Break before paragraph.
	_LOWriter_FindFormatModifyPageBreak($oDoc, $atFindFormat, $LOW_BREAK_PAGE_BEFORE)
	...
	If IsObj($oResult) Then
		...
		MsgBox($MB_OK + $MB_TOPMOST, Default, "The search was successful, I searched using a Find Format, looking for any paragraphs that have a Page Break, " & _
