...
Func Example()
	...
	If @error Then _ERROR($oDoc, $oDoc2, $oDoc3, "Failed to Create a new Base Document. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Connect to the Current Document.
	$oDoc2 = _LOBase_DocConnect($LO_DOC_CONNECT_MODE_CURRENT)
	If (@error > 0) Or Not IsObj($oDoc2) Then _ERROR($oDoc, $oDoc2, $oDoc3, "Failed to Connect to Base Document. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve Doc Name.
	$sDocName = _LOBase_DocGetName($oDoc2, False)
	If @error Then _ERROR($oDoc, $oDoc2, $oDoc3, "Failed to retrieve Base Document name. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
...
Func _ERROR($oDoc, $oDoc2, $oDoc3, $sErrorText)
	...
	If IsObj($oDoc3) Then _LOBase_DocClose($oDoc3, False)
	Exit
EndFunc
