...
Func Example()
	...
	; Set the Style's line space settings to, 1/4" above paragraph, 1/4" below paragraph,
	; Line space mode = $LOI_PAR_LINE_SPC_MODE_FIX, Line space height = 1/2".
	_LOImpress_ShapePresStyleParSpacing($oStyle, $iHMM, $iHMM, $LOI_PAR_LINE_SPC_MODE_FIX, $iHMM2)
	...
	$avSettings = _LOImpress_ShapePresStyleParSpacing($oStyle)
	If @error Then _ERROR($oDoc, "Failed to retrieve the Style's settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "The Style's Line Spacing settings are as follows: " & @CRLF & _
