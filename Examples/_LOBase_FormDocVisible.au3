...
Func Example()
	...
	; Make the Form Document invisible.
	_LOBase_FormDocVisible($oFormDoc, False)
	If @error Then Return _ERROR($oDoc, "Failed to make the form Document invisible. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; See if the Form is visible.
	$bVisible = _LOBase_FormDocVisible($oFormDoc)
	If @error Then Return _ERROR($oDoc, "Failed to query form Document visibility. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "Is the Form currently visible? True/False: " & $bVisible & @CRLF & "Press ok to make it visible.")

	; Make the Form Document visible.
	_LOBase_FormDocVisible($oFormDoc, True)
	If @error Then Return _ERROR($oDoc, "Failed to make the form Document visible. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
