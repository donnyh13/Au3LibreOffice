...
Func Example()
	Local $oDoc, $oSheet, $oCellRange, $oDestination, $oPivot, $oField, $oCell
	...
	; Set the Field/Column "2012" as a Data Field. set Subtotal to Sum.
	_LOCalc_RangePivotFieldSettings($oField, $LOC_PIVOT_TBL_FIELD_TYPE_DATA, $LOC_COMPUTE_FUNC_SUM, False)
	If @error Then _ERROR($oDoc, "Failed to set Pivot Table Field settings. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Retrieve Cell D2
	$oCell = _LOCalc_RangeGetCellByName($oSheet, "D2")
	If @error Then _ERROR($oDoc, "Failed to retrieve Cell object. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	; Modify Cell D2 to the value to 18,005.00
	_LOCalc_CellValue($oCell, 18005)
	If @error Then _ERROR($oDoc, "Failed to set Cell value. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)

	MsgBox($MB_OK + $MB_TOPMOST, Default, "I have modified the value for ""books"" for the agent, ""Michael"". It used to be 17,899.00, now it is 18,005.00." & @CRLF & _
			"Press ok to refresh the Pivot Table.")

	; Refresh the Pivot Table
	_LOCalc_RangePivotRefresh($oPivot)
	If @error Then _ERROR($oDoc, "Failed to refresh Pivot Table. Error:" & @error & " Extended:" & @extended & " On Line: " & @ScriptLineNumber)
