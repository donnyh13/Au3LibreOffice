...
Func Example()
	...
	$avPageStyleSettings = _LOWriter_PageStyleHeaderBorderWidth($oPageStyle)
	If (@error > 0) Then _ERROR("Failed to retrieve the Page style settings. Error:" & @error & " Extended:" & @extended)

	MsgBox($MB_OK, "", "The Page Style's current Header Border Width settings are as follows: " & @CRLF & _
